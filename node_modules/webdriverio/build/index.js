...
async function reloadSession(newCapabilities) {
  ...
  try {
    ...
    log10.warn(`Suppressing error closing the session: ${err.stack}`);
  }
  if (this.puppeteer?.connected) {
    this.puppeteer.disconnect();
    log10.debug("Disconnected puppeteer session");
  }
...
function scroll(x = 0, y = 0) {
  if (!x && !y) {
    return log11.warn('"scroll" command was called with no parameters, skipping execution');
  }
  if (this.isMobile) {
...
var log12 = logger12("webdriverio:throttle");
async function throttle(params) {
  log12.warn('Command "throttle" is deprecated and will be removed with the next major version release! Use `throttleNetwork` instead.');
...
async function actionClick(element, options) {
  ...
  if (x || y) {
    ...
    if (x && x < -Math.floor(width / 2) || x && x > Math.floor(width / 2)) {
      log13.warn("x would cause a out of bounds error as it goes outside of element");
    }
    if (y && y < -Math.floor(height / 2) || y && y > Math.floor(height / 2)) {
      log13.warn("y would cause a out of bounds error as it goes outside of element");
    }
  }
...
async function moveTo({ xOffset, yOffset } = {}) {
  ...
  if (xOffset || yOffset) {
    ...
    if (xOffset && xOffset < -Math.floor(width / 2) || xOffset && xOffset > Math.floor(width / 2)) {
      log15.warn("xOffset would cause a out of bounds error as it goes outside of element");
    }
    if (yOffset && yOffset < -Math.floor(height / 2) || yOffset && yOffset > Math.floor(height / 2)) {
      log15.warn("yOffset would cause a out of bounds error as it goes outside of element");
    }
  }
...
async function findDeepElement(selector) {
  ...
  const deepElementResult = await browser.browsingContextLocateNodes({ locator, context, startNodes }).then(async (result) => {
    ...
    log19.warn(`Failed to execute browser.browsingContextLocateNodes({ ... }) due to ${err}, falling back to regular WebDriver Classic command`);
  ...
  if (Object.keys(defaults).some((key) => rect[key] === void 0)) {
    ...
    Object.keys(defaults).forEach((key) => {
      ...
      if (rectJs && typeof rectJs[key] === "number") {
        ...
        log19.error("getElementRect", { rect, rectJs, key });
